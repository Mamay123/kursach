...
<html>
    ...
    <body>
        <div class='wrapper'>
            <div id='article'>
				<form action='change_maintenance_drugstore.php' method='post'>
								...
								<?php
									for($i = 0;$i < count($all); $i = $i+1)
									{
										...
										if ($result)
										{
											...
											echo "<option value='".$all[$i][0]."'>ID заказа: ".$val[$i][0].", Номер аптеки: ".$val[$i][1].", Лекарства: ".$f[0][0].", заказ: ".$val[$i][3].", ".$d[0][0].", Дата готовности: ".$val[$i][4].", Работник: ".$w[0][0]."</option>";
										}
									}
								?>
								</select><br>
